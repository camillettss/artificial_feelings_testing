...
class IA():
    ...
    def __init__(self):
        super().__init__()
        #svuota il file
        f=open('skills/latest_output.log', 'w+')
        f.close()
        self.data = json.loads(open('db/dataset.json').read())
        self.skills = skp.read_skills()
        self.feelings = json.loads(open('db/feelings.json').read())
        self.do('presentati')
        while True:
            self.run()
        ...
        if x.endswith('tu?'):
            ...
            for intent_key in self.data['intents']:
                if x in self.data['intents'][intent_key]['samples']:
                    try:
                        self.do(ipu.get_skillex(self.data['intents'][intent_key]['name']), x=x)
                        found=True
                        if self.data['intents'][intent_key]['name']=='saluto':
                            pass
                        else:
                            break
                    except Exception as e:
                        print("credo di aver commesso un errore.. :/\necco il report:\n"+str(e)+'\nerrore proveniente da: '+intent_key)
                        found=True
                        break
        if not found:
            if devmode:
    ...
    def do(self, w:str, x=''):
        ...
        if w=='reply_reask_handler':
            ...
            exec(a_script)
        elif w=='reply_usingfeela':
            y=open('skills/latest_output.log').read()
            script_descriptor = open("skills/skill_7.py")
            a_script = script_descriptor.read()
            sys.argv = ["skills/skill_7.py", x, y]
            exec(a_script)
        else:
